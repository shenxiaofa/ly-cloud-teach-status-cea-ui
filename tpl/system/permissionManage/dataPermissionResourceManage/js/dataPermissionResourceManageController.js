;(function (window, undefined) {
    ...
    var openModifyController = function ($rootScope, $timeout, $compile, $scope, $uibModalInstance, $uibModal, item, system_dataPermissionResourceManageService, alertService, formVerifyService, resource) {
        ...
        $scope.dataPermission.sjfwmc = resource.sjfwmc;
        $scope.dataPermission.sjfw = resource.sjfw;
        // system_dataPermissionResourceManageService.finddataPermissionResourceById(item.sjqxbh, function (error, message, data) {
        //     if (error) {
        //         alertService(message);
        //         return;
        //     };
    ...
    var initAddMetaData = function($scope, system_dataPermissionResourceManageService, alertService, $compile) {
        ...
        system_dataPermissionResourceManageService.findServing({pageNum: 1, pageSize: 0}, function (error, message, data) {
            if (error) {
                alertService(message);
                return;
            }
    ...
    // 初始化修改面板下拉框数据
    var initModifyMetaData = function($scope, system_dataPermissionResourceManageService, alertService, $compile) {
        $scope.fwbhData = [
            {
                bh: '',
                mc: '== 请选择 =='
        ...
        system_dataPermissionResourceManageService.findServing({pageNum: 1, pageSize: 0}, function (error, message, data) {
            ...
            var html = '' +
                '<select ng-model="dataPermissionResource.fwbh" ui-chosen="dataPermissionResourceModifyForm.fwbh" ng-required="true" id="fwbh_select" name="fwbh" ui-jq="chosen" ui-options="{search_contains: true}" class="form-control">' +
                '<option ng-repeat="fwbhItem in fwbhData" value="{{fwbhItem.bh}}">{{fwbhItem.mc}}</option>' +
                '</select>';
            angular.element("#fwbh_select").parent().empty().append(html);
            $compile(angular.element("#fwbh_select").parent().contents())($scope);
